...
processAffy = function(obiekt){
  ...
  if(class(obiekt) == 'AffyBatch'){
    ...
    # normalizacja
    obiekt_znorm = rma(obiekt, background=TRUE, normalize=TRUE, subset=NULL, target="core")
    
    # adnotacja
    wyjscie = exon_annot(obiekt_znorm)
    
  }else if(class(obiekt) == 'try-error'){
    
    wyjscie == 'There were no data to normalize'
    warning("There are no data to normalize.")
    
  }else{
    
    wyjscie = "This object class is not supported for chosen platform."
    warning(paste("This object class (", as.character(class(obiekt)), ") could not be normalized with norm_and_annot_function with platform argument Affymetrix. 
                  Supported classes for this function for Affymetrix platform are AffyBatch and ExonFeatureSet or list of AffyBatch and ExonFeatureSet objects.", sep=' '))
  }
